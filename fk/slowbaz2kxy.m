...
%
%    Description: [Kx,Ky]=SLOWBAZ2KXY(S,BAZ,F) converts slowness S,
%     backazimuth BAZ and frequency F into wavenumbers in the East (Kx) and
%     North (Ky) directions.  Positive Kx correspond to waves from the East
%     heading West and positive Ky to waves from the North going South.
%     The units of Kx & Ky depend on the units of S & F.  The distance
%     units of Kx & Ky will match that of S, so if S is in seconds per
%     degree then Kx & Ky will be in wavelengths per degree.  If F is an
%     angular frequency then Kx & Ky will be in radians per distance unit.
%     If F is not given, it is assumed to be 1Hz.
%
%    Notes:
...
%
%     Wavenumbers in rad/km for a 10s wave moving at 4.43s/deg from 312deg:
