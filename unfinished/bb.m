...
if D > 0
   ...
   yy = ampy*y*D/90 + CY;
   if ta == 0
      fill(xx,yy,'w','tag','bb_background')
      fill(Y,X,color,'tag','bb_tension')
      line(xx,yy,'color','k','linewidth',0.2,'tag','bb_outline');
   else
      fillm(yy,xx,'w')
      fillm(X,Y,color)
...
D = [d(3,3) d(1,1) d(2,2)];
V(2:3,1:3) = -V(2:3,1:3);
V = [V(2,3) V(2,1) V(2,2); V(3,3) V(3,1) V(3,2); V(1,3) V(1,1) V(1,2)]; % gge debug

IMAX = find(D == max(D)); % gge debug
IMIN = find(D == min(D)); % gge debug
...
if (abs(ZN) < AAA)
	...
	SD=sin(FDH);
	if  (SD == 0)
        % this is just wrong (gge debug)
        %error('fail by Chen Ji') % gge debug
        FT=0;
        FL=0;
		return;
	end 
