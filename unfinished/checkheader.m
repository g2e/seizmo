...
%CHECKHEADER    Check and update header field/values of SEIZMO records
%
...
%
%     CHECKHEADER(DATA,OPTIONS) allows choosing which checks are performed.  The
...
%           data=checkheader(data,options,'field1',values1,...,'fieldN',valuesN)
%
%    Examples:
...
if(any([year jday hour minute sec msec]...
    ...
    error('seizmo:checkheader:badRefTime','NZ fields must be integers!');
end
...
if(~isempty(intersect(option,{'timing' 'all'})))
    ...
    if(any(b>e))
        error('seizmo:checkheader:badTiming',...
    ...
    if(any(check))
        for i=find(check).'
            if(gh(data(i),iztype{i}(2:end))~=0)
                warning('seizmo:checkheader:badIZTYPE',...
...
if(~isempty(intersect(option,{'vsdata' 'all'})))
    ...
    for i=1:nrecs
        ...
        if(~isfield(data,'dep'))
            error('seizmo:checkheader:noDEP',...
