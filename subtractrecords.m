...
%SUBF    Subtract SEIZMO data records
%
...
if(mod(nargopt,2))
    error('seizmo:subf:badNumOptions','Unpaired option(s)!');
end
...
for i=1:2:nargopt
    ...
    if(isfield(option,varargin{i}))
        ...
        warning('seizmo:subf:badInput','Unknown Option: %s !',varargin{i}); 
    end
end
...
if(any(nrecs~=1 & nrecs~=maxrecs))
    error('seizmo:subf:nrecsMismatch',...
...
if(ndatasets>1)
    % check records
    if(~isequal(iftype{:}))
        report.identifier='seizmo:subf:mixedIFTYPE';
    ...
    for i=1:ndatasets
        if(any(~strcmpi(leven{i},'true')))
            report.identifier='seizmo:subf:illegalOperation';
    ...
    if(~isequal(ncmp{:}))
        report.identifier='seizmo:subf:mixedNCMP';
    ...
    if(~isequal(npts{:}))
        report.identifier='seizmo:subf:mixedNPTS';
    ...
    if(~isequal(delta{:}))
        report.identifier='seizmo:subf:mixedDELTA';
    ...
    if(~isequal(b{:}))
        report.identifier='seizmo:subf:mixedB';
    ...
    if(~isequal(nzyear{:}) || ~isequal(nzjday{:}) ...
        ...
        report.identifier='seizmo:subf:mixedReferenceTimes';
