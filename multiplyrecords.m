...
%MULF    Multiply SEIZMO data records
%
...
if(mod(nargopt,2))
    error('seizmo:mulf:badNumOptions','Unpaired option(s)!')
end
...
for i=1:2:nargopt
    ...
    if(isfield(option,varargin{i}))
        ...
        warning('seizmo:addf:badInput','Unknown Option: %s !',varargin{i}); 
    end
end
...
if(any(nrecs~=1 & nrecs~=maxrecs))
    error('seizmo:mulf:nrecsMismatch',...
...
if(ndatasets>1)
    % check records
    if(~isequal(iftype{:}))
        report.identifier='seizmo:mulf:mixedIFTYPE';
    ...
    for i=1:ndatasets
        if(any(~strcmpi(leven{i},'true')))
            report.identifier='seizmo:mulf:illegalOperation';
    ...
    if(~isequal(ncmp{:}))
        report.identifier='seizmo:mulf:mixedNCMP';
    ...
    if(~isequal(npts{:}))
        report.identifier='seizmo:mulf:mixedNPTS';
    ...
    if(~isequal(delta{:}))
        report.identifier='seizmo:mulf:mixedDELTA';
    ...
    if(~isequal(b{:}))
        report.identifier='seizmo:mulf:mixedB';
    ...
    if(~isequal(nzyear{:}) || ~isequal(nzjday{:}) ...
        ...
        report.identifier='seizmo:mulf:mixedReferenceTimes';
