...
%     layout is:
%          .response - complex-valued fk data
...
%        June  3, 2010 - response is now complex & single, drop .normdb
%
%     Written by Garrett Euler (ggeuler at wustl dot edu)
%     Last Updated June  3, 2010 at 18:25 GMT
...
try
    ...
    for a=1:nrng
        ...
        for b=1:nfreq
            ...
            switch center
                ...
                otherwise
                    if(nfreq==1)
                        smap(a).response(:)=smap(a).response(:)...
                            +exp(f(fidx(b))*p)*cs(:,b).^2;
                    else
                        % we lose the complex response to average
                        smap(a).response(:)=smap(a).response(:)...
        ...
        % - convert sum to mean
        % - normalize by number of position vectors
        % - convert to complex (if necessary)
        smap(a).response=complex(smap(a).response/(nfreq*nidx));
