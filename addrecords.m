...
if(mod(nargopt,2))
    error('seizmo:addf:badNumOptions','Unpaired option(s)!');
end
...
if(any(nrecs~=1 & nrecs~=maxrecs))
    error('seizmo:addf:nrecsMismatch',...
...
if(ndatasets>1)
    % check records
    if(~isequal(iftype{:}))
        report.identifier='seizmo:addf:mixedIFTYPE';
    ...
    for i=1:ndatasets
        if(any(~strcmpi(leven{i},'true')))
            report.identifier='seizmo:addf:illegalOperation';
    ...
    if(~isequal(ncmp{:}))
        report.identifier='seizmo:addf:mixedNCMP';
    ...
    if(~isequal(npts{:}))
        report.identifier='seizmo:addf:mixedNPTS';
    ...
    if(~isequal(delta{:}))
        report.identifier='seizmo:addf:mixedDELTA';
    ...
    if(~isequal(b{:}))
        report.identifier='seizmo:addf:mixedB';
    ...
    if(~isequal(nzyear{:}) || ~isequal(nzjday{:}) ...
        ...
        report.identifier='seizmo:addf:mixedReferenceTimes';
