...
%
%    Description: [LAT,LON]=GC2LATLON(LAT1,LON1,LAT2,LON2) calculates
%     points LAT/LON along the great circle(s) defined by LAT1/LON1 and
%     LAT2/LON2 at increments of 1 degree.  All LAT/LON arguments must
%     either be scalar or arrays with the same number of elements.  Outputs
%     are returned so that each row corresponds to a separate great circle
%     given by the input arguments.  All inputs must be in degrees!
%
...
if(~isequal(sz1,sz2) || ~isequal(sz3,sz4))
    error('seizmo:gc2latlon:nonscalarUnequalArrays',...
...
lon=nan(n1,npts);
for i=1:n1
    [lat(i,:),lon(i,:)]=sphericalfwd(lat1(i),lon1(i),steps,az(i));
end

% sort so longitudes go from smallest to largest
% - this is so plots look clean (no line across map hopefully)
[lon,i]=sort(lon,2);
lat=matchsort(lat,i,2);
