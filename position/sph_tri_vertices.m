...
%
%    Description: VERTICES=SPH_TRI_VERTICES(A,B,C) extracts the unique
%     vertices from the spherical triangle polyhedra formed by A, B, & C.
%     VERTICES is an Mx3 array of xyz values where M is the number of
%     vertices.  If A, B, & C are Nx3 arrays (where N is the number of
%     faces of the polyhedra) then M should be N/2+2.
%
%    Notes:
...
%     Last Updated Nov. 17, 2009 at 08:00 GMT
...
if(size(vertices,1)~=(sa/2+2))
    % something is wrong
    warning('seizmo:sph_tri_vertices',...
