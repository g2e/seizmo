...
try
    ...
    undef=getsubfield(h(idx),'undef','ntype').';
    ...
    if(option.USEABSOLUTETIMING)
        [b,e,delta,npts,ncmp,nz,kcmpnm,cmpinc,cmpaz]=getheader(data,...
            'b','e','delta','npts','ncmp','nz','kcmpnm','cmpinc','cmpaz');
    else
        [b,e,delta,npts,ncmp,kcmpnm,cmpinc,cmpaz]=getheader(data,...
            'b','e','delta','npts','ncmp','kcmpnm','cmpinc','cmpaz');
        nz=nan(nrecs,6);
    end
    ...
    for i=1:npairs
        ...
        if(option.DEBUG)
            ...
            disp(sprintf('Number in Group: %d',np));
        end
        ...
        if(option.DEBUG)
            disp(' ');
            disp(sprintf('Finished Rotating Group: %d',i));
            disp(['Members: ' sprintf('%d ',oldc2+1:c2)]);
            disp(sprintf('Number in Group: %d',c2-oldc2));
        end
