function [ax,sx,sy,st,sc]=adjust_monthly_plots(fh,mytitle)
...
if(any(~ishandle(fh)) && strcmp(unique(get(fh,'tag')),'fkmonthly'))
    error('seizmo:adjust_monthly_plots',...
...
for i=1:nb
    ...
    print(['-f' num2str(fh(i))],'-dpdf',...
        ['fkmonthly_' lower(cmp) '_' num2str(bands(i,2)) ...
        's-' num2str(bands(i,1)) 's_' zerodb '_' num2str(dblim(1)) ...
        'db-' num2str(dblim(2)) 'db.pdf']);
    saveas(fh(i),['fkmonthly_' lower(cmp) '_' num2str(bands(i,2)) ...
        's-' num2str(bands(i,1)) 's_' zerodb '_' num2str(dblim(1)) ...
