...
%    Usage:    plotww3(s)
%              plotww3(s,rng)
%              plotww3(s,rng,fgcolor,bgcolor)
%              plotww3(s,rng,fgcolor,bgcolor,ax)
%              ax=plotww3(...)
%
%    Description:
%     PLOTWW3(S) plots the WaveWatch III data contained in the structure S
%     created by WW3STRUCT.  The plots average the data across time (if you
%     did not specify a specific GRiB "record" or time).  One plot per
%     data type (wind u/v components are drawn seperately).
%
%     PLOTWW3(S,RNG) sets the colormap limits for the data. The default
%     is [0 15] which works well for significant wave heights.  A scale of
%     [0 25] will prevent saturation of wave heights & periods.
%
%     PLOTWW3(S,RNG,FGCOLOR,BGCOLOR) specifies foreground and
...
%
%     PLOTWW3(S,RNG,FGCOLOR,BGCOLOR,AX) sets the axes to draw in.  This
%     is useful for subplots, guis, etc.  AX must have the same number of
%     elements as S.DATA (as in one plot per data type).  The default
